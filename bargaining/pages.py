...
class Bargaining(Page):
    ...
    def is_displayed(self):
        if self.player.is_worker():
            return self.player.participant.vars.get('wants_to_negotiate', False)
        else:
            partner = self.player.get_partner()
            return partner and partner.participant.vars.get('wants_to_negotiate', False)
    ...
    def vars_for_template(self):
        ...
        return {
            'has_ai_expert': self.player.participant.vars.get('has_ai_expert', False),
            'is_worker': self.player.role() == 'Worker',
            'suggested_wage': self.participant.vars.get('suggested_wage')
        }
    ...
    def live_method(player, data):
        group = player.group
        other = group.get_players()[1] if group.get_players().index(player) == 0 else group.get_players()[0]
        ...
        # Rest of the live_method remains same
