...
class MatrixTask(Page):
    ...
    def live_method(player, data):
        if 'index1' in data and 'index2' in data:
            ...
            player.time_spent = data.get('time_spent', 0)
...
class NegotiationDecision(Page):
    ...
    def before_next_page(self):
        # Store decision in participant vars
        self.participant.vars['wants_to_negotiate'] = self.player.wants_to_negotiate
        self.participant.vars['suggested_wage'] = self.player.suggested_wage

        # Update paired recruiter
        partner = self.player.get_partner()
        if partner:
            partner.participant.vars['next_app'] = 'bargaining' if self.player.wants_to_negotiate else 'questionnaire'
            partner.participant.vars['suggested_wage'] = self.player.suggested_wage
...
class PostQuestionnaire(Page):
    def is_displayed(self):
        ...
        if self.player.is_worker():
            return not self.player.wants_to_negotiate
        else:  # is recruiter
            worker = self.player.get_partner()
            return not worker.wants_to_negotiate
...
class AcceptedWageResults(Page):
    def is_displayed(self):
        ...
        if self.player.is_worker():
            ...
            partner = self.player.get_partner()
            return not partner.wants_to_negotiate if partner else False

    def vars_for_template(self):
        partner = self.player.get_partner()
        return {
            'is_worker': self.player.is_worker(),
            'suggested_wage': self.player.suggested_wage if self.player.is_worker() else partner.suggested_wage,
            'joint_revenue': self.group.calculate_joint_revenue(),
        }
...
page_sequence = [MatrixTask, FinalWaitPage, Results, NegotiationDecision, DecisionWaitPage]
