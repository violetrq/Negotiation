        ...
        if self.round_number == 1:
            for group in self.get_groups():
                for player in group.get_players():
                    is_worker = player.id_in_group <= 2
                    player.participant.vars['role'] = 'Worker' if is_worker else 'Recruiter'
                    # Explicit pairing based on ID
                    if player.id_in_group == 1:
                        player.paired_with = 3
                    elif player.id_in_group == 2:
                        player.paired_with = 4
                    elif player.id_in_group == 3:
                        player.paired_with = 1
                    elif player.id_in_group == 4:
                        player.paired_with = 2
                    player.participant.vars['paired_with'] = player.paired_with
...
class Player(BasePlayer):
    ...
    paired_with = models.IntegerField()  # Add initial value
    # Existing fields
